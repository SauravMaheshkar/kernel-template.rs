...
struct Buffer {
    chars:
        [[Volatile<ScreenChar>; crate::constants::BUFFER_WIDTH]; crate::constants::BUFFER_HEIGHT],
}
...
impl Writer {
    ...
    pub fn write_byte(&mut self, byte: u8) {
        match byte {
            b'\n' => self.new_line(),
            byte => {
                if self.column_position >= crate::constants::BUFFER_WIDTH {
                    self.new_line();
                }

                let row = crate::constants::BUFFER_HEIGHT - 1;
    ...
    fn new_line(&mut self) {
        for row in 1..crate::constants::BUFFER_HEIGHT {
            for col in 0..crate::constants::BUFFER_WIDTH {
                let character = self.buffer.chars[row][col].read();
                self.buffer.chars[row - 1][col].write(character);
            }
        }
        self.clear_row(crate::constants::BUFFER_HEIGHT - 1);
...
fn test_println_output() {
    ...
    let s = "life finds a way";
    interrupts::without_interrupts(|| {
        let mut writer = WRITER.lock();
        writeln!(writer, "\n{}", s).expect("writeln failed");
        for (i, c) in s.chars().enumerate() {
            let screen_char = writer.buffer.chars[crate::constants::BUFFER_HEIGHT - 2][i].read();
